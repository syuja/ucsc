...
mkdir -p $MYSQLDATA
rmdir /var/lib/mysql
#shouldnt need this, why is is looking for socket here? look in hg.conf
...
mkdir -p $SWDIR/bin/$MACHTYPE && ln -s $SWDIR/bin ~/bin

#grab and compile samtabix
git clone http://genome-source.cse.ucsc.edu/samtabix.git $SWDIR/samtabix && cd $SWDIR/samtabix && make

#grab and compile kent source
cd $SWDIR && curl -sLo master.zip https://github.com/ucscGenomeBrowser/kent/archive/master.zip && unzip -q master.zip && mv kent-master kent && rm -f master.zip
sed -i 's/hgBeacon//g' $SWDIR/kent/src/hg/makefile && sed -i 's/hgMirror//g' $SWDIR/kent/src/hg/makefile #hgMirror makefile breaks cgi make - -${USER} issue
